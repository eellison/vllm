...
When compiled, Inductor fuses this with adjacent operations. On SM100+
(B200), uses inline_asm_elementwise to emit a single PTX
cvt.rni.sat.s8.f32 instruction for the saturating conversion. On SM90
(H100), the inline asm causes ~4x register bloat in the generated Triton
kernel (pack=1 prevents register reuse), so we use the pure-torch
round+clamp+cast fallback which Triton optimizes well.
"""
...
def _use_inline_asm() -> bool:
    """Use inline asm only on SM100+ where it doesn't cause register bloat."""
    if not _HAS_INLINE_ASM:
        return False
...
_USE_INLINE_ASM = _use_inline_asm()


def _saturating_int8_cast(x: torch.Tensor) -> torch.Tensor:
    """Saturating float32 -> int8 cast with round-to-nearest and clamping.

    Uses PTX inline asm on SM100+ (1 instruction vs 3 for the fallback).
    On SM90, the inline asm causes excessive register pressure in Triton's
    codegen, so we use the pure-torch fallback which Triton optimizes well.
    """
    if _USE_INLINE_ASM:
